...
$apitext = callAPI("gbshop" , "member_validateZaloLogin.api", array(), $params);
...
if($apiData['status'] == 'done') {
    ...
    $url = HTTP_SERVER . 'gbshop/login.gbe?userid=' . $dataSession['membercode'] . '&email=' . $dataSession['loginmemberemail'] . '&name=' . $dataSession['loginmembername'] . '&avatar=' . $dataSession['loginmemberavatar'] . '&phone=' . $dataSession['loginmemberphone'] . '&type=zalo';
    header("Location:" . $url);
}
?>
